...
from app.archives.controlled_copies import ensure_copy_usable
...
class DossierLifecycleService:
    ...
    def disclose(self, principal: Principal, dossier_id: int, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if data.get("controlled_copy_id"):
            ensure_copy_usable(self.connection, data["controlled_copy_id"], dossier_id, self.clock.now())
...
class AccessLoanService:
    ...
    def create(self, principal: Principal, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if data.get("controlled_copy_id"):
            ensure_copy_usable(self.connection, data["controlled_copy_id"], data["dossier_id"], self.clock.now())
        now = to_storage(self.clock.now())
        access_code = data.get("access_code") or f"LOAN-{uuid.uuid4().hex[:12]}"
        cursor = self.connection.execute(
            """INSERT INTO access_loans(access_code,dossier_id,requester_user_id,quantity,due_at,controlled_copy_id,state,created_at,updated_at)
               VALUES(?,?,?,?,?,?,'active',?,?)""",
            (access_code, data["dossier_id"], data["requester_user_id"], data["quantity"], data["due_at"], data.get("controlled_copy_id"), now, now),
        )
        self.connection.execute(
